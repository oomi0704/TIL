...
> 学習メモ（2026-05-11 〜 05-21）を統合した個人用まとめ  
...
| **NLB** | TCP/UDP、超低レイテンシ |
| **GLB** | サードパーティ仮想アプライアンス（ファイアウォール等） |

> **アプライアンス**: 特定機能に特化した専用機器。AWSではGLBでサードパーティ製品をVPCに組み込む。オンプレFW代替は **AWS Network Firewall**
...
- **Amplify**: React/Vue/Next.js/Flutter等を素早くデプロイ
...
| 項目 | 内容 |
|---|---|
| 仕組み | 属性にUnix時刻（秒）を設定し期限切れアイテムを**自動削除** |
| 削除タイミング | 最大**48時間以内**（即時ではない） |
| コスト | TTL削除は**WCUを消費しない** |
| 用途 | セッション・OTP・一時キャッシュデータ |
...
| セッション・ランキング | **Redis** |
| セッション永続化・マルチAZ | **ElastiCache Redis** |

| | DAX | ElastiCache Redis | Memcached |
|---|---|---|---|
| 対象 | DynamoDBのみ | RDS・DynamoDB・EC2アプリ | 汎用キャッシュ |
| アプリ変更 | ほぼ不要 | 実装必要 | 実装必要 |
| 永続化 | ❌ | ✅ | ❌ |
| 高可用性・マルチAZ | — | ✅ | ❌ |
| スケール | — | 垂直中心 | 水平・シンプル |
...
### 4.7 Amazon Redshift（分析・DWH）

| 項目 | 内容 |
|---|---|
| 種類 | **データウェアハウス**（分析・集計向け） |
| 特徴 | 列指向ストレージ・MPP（並列処理）で大量データを高速分析 |
| vs RDS | 日常トランザクション → **RDS** / ペタバイト級分析 → **Redshift** |
| 可用性 | **マルチAZ対応**（RA3ノードタイプ） |
| **Redshift Spectrum** | S3上のデータを直接クエリ |

- Kinesis Firehoseの宛先にも頻出
...
| エッジで何をするか | コンテンツを**キャッシュ**配信 | エッジを**経由**しAWS網で高速化 | エッジに**DNS**配置 |
| キャッシュ | あり | なし | — |
...
|---|---|
| **CloudFront** | キャッシュ配信 |
| **Global Accelerator** | 経由してAWS内部網で高速化 |
| **Route 53** | DNSサーバー配置 |
| **S3 Transfer Acceleration** | エッジ経由でS3へ高速アップロード |
| **WAF** | 不正リクエストをブロック |
| **Shield** | DDoS防御 |
...
|---|---|
| シンプル | 1つのIPを返す |
| レイテンシーベース | 最も**応答が速い**リージョンへ |
| 地理的（Geolocation） | ユーザーの**地域で固定**振り分け |
| **地理的近接性（Geoproximity）** | 地理的に**最も近い**リソースへ |
| 加重（Weighted） | 割合で分散 |
| フェイルオーバー | 障害時にスタンバイへ（**メンテナンスページ切替**にも） |

> Geolocation = 地域で固定 / Geoproximity = 近い方へ動的

### 6.3 カスタムエラーページ

| シナリオ | 答え |
|---|---|
| 404・500をカスタムページに | **CloudFront**（カスタムエラーレスポンス） |
| サーバーダウン時にメンテナンスページへ | **Route 53 フェイルオーバー** |

### 6.4 WAF・Shield・Geo制限
...
| | IAM | SCP |
|---|---|---|
| スコープ | ユーザー・グループ・ロール（**人単位**） | Organizationsのアカウント・OU（**アカウント単位**） |
| 役割 | 権限を**付与** | **最大権限の境界**を設定 |
| 実効権限 | SCPとIAMの**積集合**（両方で許可された範囲のみ） |
...
| 重複排除 | — | ✅ |
| スループット | 無制限 | 制限あり |
| 保持期間 | デフォルト4日・最大**14日** | 同左 |
| メッセージサイズ | 最大**256KB** | 同左 |
| インフライト上限 | **120,000件** | **20,000件** |

| 要件 | 答え |
|---|---|
| 順序保証・重複排除が必要 | **SQS FIFO** |
...
| **DataSync** | オンプレ↔AWS ファイル同期（S3・EFS） | 定期同期 |
| **Storage Gateway** | オンプレとAWSの恒常ハイブリッド | File/Vol/Tape |
| **Snowball** | 大量データの物理輸送 | 10TB+・ネット遅い |
| **Snowmobile** | 超大量（**100PB以上**） | 物理輸送 |
| **DMS** | DB移行・**ダウンタイムなし**・リアルタイム同期 | データベース |
| **SCT + DMS** | **異なるDBエンジン間**の移行 | エンジン変換 |
| **mysqldump + S3** | 小規模MySQLのシンプル移行 | 小規模 |
| **S3 Transfer Acceleration** | S3アップロード高速化 | 大容量アップロード |

**Snowball を選ぶ条件**

- データ量 **10TB以上** かつ ネットワークが**遅い・不安定**
- 1Gbps回線で**1週間以上**かかる見込み → Snowball検討
- **100PB以上** → Snowmobile
...
| ダウンタイムなしDB移行 | DMS |
| 異なるDBエンジン間移行 | SCT + DMS |
| 10TB+・ネット遅い移行 | Snowball |
| データ分析・DWH | Redshift |
| 順序保証・重複排除 | SQS FIFO |
| TTL・自動削除・OTP | DynamoDB TTL |
| 404/500カスタムページ | CloudFront |
| メンテナンスページ切替 | Route 53 フェイルオーバー |
| 地理的に最も近いDNS | Geoproximity |
...
| Geolocation vs Geoproximity | 地域固定 vs 最も近い |
| IAM vs SCP | 人に付与 vs アカウント上限・積集合 |
| RDS vs Redshift | トランザクション vs 分析・DWH |
| CloudFront vs Route53エラー | HTTPエラーページ vs メンテ切替 |
| SQS標準 vs FIFO | 順不同・無制限 vs 順序・重複排除 |
...
- [ ] DynamoDB TTLは48時間以内・WCU消費なし
- [ ] RDS Multi-AZのRPO≈0・RTO 1-2分（Auroraは30秒）
- [ ] Snowballは10TB+・1Gbpsで1週間超の目安
- [ ] エラーページはCF、メンテ切替はRoute53 FO
- [ ] Geolocation（固定）と Geoproximity（近い）を区別

---

*出典: 2026-05-11 〜 05-21 学習メモ（[[2026-05-21]] 追記分含む）*
